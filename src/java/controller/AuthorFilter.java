        ...
        try {
            ...
            for(Cookie co : req.getCookies()){
                if((co.getName()).compareTo("loginname") == 0){
                    cc = co;
                    if(!(dao.getAccountByUsername(cc.getValue()) != null)){
                        session.setAttribute("error", "Something wrong happens, login againnn");  
                        co.setMaxAge(0);
                        res.addCookie(co);
                        res.sendRedirect("/1849/login.jsp");
                    }
                                     
                }
                if((co.getName()).compareTo("logininfo") == 0){
                    ccc = co;
                }
            }  
            if(cc == null){                
                session.setAttribute("error", "Please login");
                res.sendRedirect("/1849/report.jsp");
            }
            else{
                String decrypt = dao.mySuperSecureDecrypt(dao2.decode(ccc.getValue()));
                String user = decrypt.substring(0, decrypt.length()/2);
                String pwd = decrypt.substring(decrypt.length()/2, decrypt.length());
                               
                
                Account_ngoc acc = dao.getAccountByUsername(cc.getValue());
                
                if(!dao.HashMD5(acc.getUsr()).equals(user) || !acc.getPwd().equals(pwd)){
                    session.setAttribute("error", "Something wrong happens, login againnn");           
                    res.sendRedirect("/1849/login.jsp");
                }
                ...
                //Account acc = dao.getAccountByUsername(cc.getValue());
                // controller.account
                
                String role = acc.getRole() == 1 ? "admin" : acc.getRole() == 2 ? "user" : "shipper";              
               
                if(!url.startsWith(role) || acc == null ){
                    session.setAttribute("error", cc.getValue() + " don't have permission to access this page");
                    res.sendRedirect("/1849/report.jsp");
                }
            ...
            problem = t;
            //t.printStackTrace();
            session.setAttribute("error", "Something wrong happens, login again");           
            res.sendRedirect("/1849/login.jsp");
        }
